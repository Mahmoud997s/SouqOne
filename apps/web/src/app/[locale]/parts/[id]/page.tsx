...
function PhotoGrid({ images, title, onOpen }: { images: ImgItem[]; title: string; onOpen: (i: number) => void }) {
  if (images.length === 0) return (
    <div className="rounded-2xl overflow-hidden bg-surface-container-high hidden md:flex items-center justify-center" style={{ height: 360 }}>
      <span className="material-symbols-outlined text-7xl text-on-surface-variant/30">build</span>
    </div>
  );
  return (
...
function MobileSwiper({ images, title, onOpen }: { images: ImgItem[]; title: string; onOpen: (i: number) => void }) {
  ...
  if (images.length === 0) return (
    <div className="h-64 bg-surface-container-high flex items-center justify-center md:hidden">
      <span className="material-symbols-outlined text-6xl text-on-surface-variant/30">build</span>
    </div>
  );
  return (
...
export default function PartDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const COND_COLORS: Record<string, string> = {
    ...
    USED: 'bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-800',
    REFURBISHED: 'bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800',
  };
  ...
  if (error || !part) return (
    <><Navbar /><div className="min-h-screen pt-28"><main className="max-w-5xl mx-auto px-4 md:px-8"><ErrorState message={tp('partDetailNotFound')} /></main></div><Footer /></>
  );

  const images: ImgItem[] = (part.images ?? []).map(img => ({ id: img.id, url: getImageUrl(img.url) || '' })).filter(i => i.url);
  const isOwner = user?.id === part.seller?.id;
  const sellerName = part.seller?.displayName || part.seller?.username || '';
  const detailRows = [
    part.partNumber ? { label: tp('partDetailPartNumber'), value: part.partNumber } : null,
    part.compatibleMakes?.length > 0 ? { label: tp('partDetailMakes'), value: part.compatibleMakes.join(', ') } : null,
    part.yearFrom ? { label: tp('partDetailYears'), value: `${part.yearFrom}${part.yearTo ? ` - ${part.yearTo}` : '+'}` } : null,
    part.governorate ? { label: tp('partDetailLocation'), value: `${part.governorate}${part.city ? `، ${part.city}` : ''}` } : null,
        ...
        <main className="max-w-5xl mx-auto px-4 md:px-8 pt-16 pb-28 lg:pb-16">
          ...
          <div className="flex items-center justify-between mb-5">
            <nav className="flex items-center gap-1 text-[12px] text-on-surface-variant flex-wrap">
              ...
              <Link href="/parts" className="text-primary hover:underline">قطع الغيار</Link>
              <span className="mx-0.5">›</span>
              <span className="truncate max-w-[160px]">{CAT_LABELS[part.partCategory] || part.partCategory}</span>
            </nav>
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <span className={`px-3 py-0.5 rounded-full text-[11px] font-medium border ${COND_COLORS[part.condition] || 'bg-surface-container-low text-on-surface-variant border-outline-variant/30'}`}>
                {COND_LABELS[part.condition] || part.condition}
              </span>
              {part.isOriginal && <span className="px-3 py-0.5 rounded-full text-[11px] font-medium bg-primary/10 text-primary border border-primary/20">أصلية</span>}
            </div>
            <h1 className="text-[24px] font-bold text-on-surface mb-2 leading-tight">{part.title}</h1>
            <div className="flex items-center gap-2 flex-wrap text-[12px] text-on-surface-variant">
              {part.governorate && <span>{part.governorate}{part.city ? `، ${part.city}` : ''}، عُمان</span>}
              {part.viewCount > 0 && (
                <>
          ...
          <div className="md:hidden -mx-4 mb-5">
            <MobileSwiper images={images} title={part.title} onOpen={setLightboxIdx} />
          </div>

          {/* ══ PHOTO GRID ══ */}
          <div className="mb-8">
            <PhotoGrid images={images} title={part.title} onOpen={setLightboxIdx} />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8 items-start">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 py-1 pb-3">
                <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-primary/20 bg-surface-container-high flex items-center justify-center flex-shrink-0">
                  {part.seller?.avatarUrl ? (
                    <Image src={getImageUrl(part.seller.avatarUrl) || ''} alt={sellerName} width={48} height={48} className="object-cover w-full h-full" />
                  ) : (
                ...
                <div>
                  ...
                  {part.seller?.createdAt && (
                    <p className="text-[11px] text-on-surface-variant mt-0.5">
                      عضو منذ {new Date(part.seller.createdAt).toLocaleDateString(locale === 'ar' ? 'ar-OM' : 'en-US', { year: 'numeric', month: 'long' })}
                    </p>
                  )}
                </div>
              </div>
              ...
              {part.description && (
                <>
                  <div>
                    <SectionTitle>وصف القطعة</SectionTitle>
                    <p className="text-[13px] text-on-surface-variant leading-relaxed whitespace-pre-line">{part.description}</p>
                  </div>
                  <Divider />
                </>
              )}
              ...
              {detailRows.length > 0 && (
                <>
                  <div>
                    <SectionTitle>تفاصيل القطعة</SectionTitle>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {detailRows.map((row, i) => (
                        <div key={i} className="flex flex-col gap-0.5 px-4 py-3 rounded-2xl border border-primary/10 bg-gradient-to-br from-primary/5 to-primary/10 hover:border-primary/30 transition-all duration-200">
                          ...
                          <span className="text-[12px] text-on-surface-variant">{row.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {part.latitude && part.longitude && (
                <div>
                  ...
                  <p className="text-[12px] text-on-surface-variant mb-3 flex items-center gap-1.5">
                    ...
                    {part.governorate}{part.city ? `، ${part.city}` : ''}، عُمان
                  </p>
              ...
              <div className="border border-outline-variant/20 rounded-2xl p-5 shadow-sm bg-surface-container-lowest">
                <div className="mb-4">
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-[26px] font-bold text-on-surface">{parseFloat(part.price).toFixed(3)}</span>
                    <span className="text-[14px] text-on-surface-variant">ر.ع</span>
                  </div>
                  {part.isPriceNegotiable && (
                    <span className="text-[11px] text-on-surface-variant bg-surface-container px-2.5 py-1 rounded-full inline-block mt-1">قابل للتفاوض</span>
                  )}
                </div>
                ...
                {!isOwner && (
                  <div className="space-y-2.5">
                    <button onClick={handleMessage} disabled={createConv.isPending}
                      ...
                      {createConv.isPending ? 'جاري...' : 'مراسلة البائع'}
                    </button>
                    {part.contactPhone && (
                      <a href={`tel:${part.contactPhone}`}
      ...
      {!isOwner && (
        <div className="fixed bottom-0 inset-x-0 z-50 lg:hidden bg-background/95 backdrop-blur-sm border-t border-outline-variant/30 px-4 py-3 pb-[env(safe-area-inset-bottom)]">
          <div className="flex items-baseline gap-1 mb-2">
            <span className="text-[15px] font-semibold text-on-surface">{parseFloat(part.price).toFixed(3)} ر.ع</span>
            {part.isPriceNegotiable && <span className="text-[12px] text-on-surface-variant">· قابل للتفاوض</span>}
          </div>
