        ...
        <main className="max-w-5xl mx-auto px-4 md:px-8 -mt-20 md:-mt-24 relative z-10 pb-16">
          <nav className="flex items-center gap-2 text-sm text-white/70 mb-5">
            <Link href="/trips" className="hover:text-white transition-colors flex items-center gap-1">
              <span className="material-symbols-outlined text-sm">directions_bus</span> رحلات
            </Link>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-surface-container-lowest dark:bg-surface-container border border-outline-variant/10 dark:border-outline-variant/20 overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-outline-variant/10 dark:border-outline-variant/20 flex items-center gap-2">
                  ...
                  <h2 className="font-black text-on-surface">الجدول الزمني</h2>
                </div>
                <div className="p-6 space-y-4">
                  {trip.operatingDays.length > 0 && (
                    <div>
                      <p className="text-xs text-on-surface-variant mb-2 font-bold">أيام التشغيل</p>
                  ...
                  {trip.departureTimes.length > 0 && (
                    <div>
                      <p className="text-xs text-on-surface-variant mb-2 font-bold">أوقات المغادرة</p>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {(trip.startDate || trip.endDate) && (
                    <div className="flex gap-6 pt-3 border-t border-outline-variant/10 dark:border-outline-variant/20">
                      {trip.startDate && <div><p className="text-xs text-on-surface-variant">من تاريخ</p><p className="font-black text-sm text-on-surface">{new Date(trip.startDate).toLocaleDateString('ar-OM')}</p></div>}
                      {trip.endDate && <div><p className="text-xs text-on-surface-variant">إلى تاريخ</p><p className="font-black text-sm text-on-surface">{new Date(trip.endDate).toLocaleDateString('ar-OM')}</p></div>}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-surface-container-lowest dark:bg-surface-container border border-outline-variant/10 dark:border-outline-variant/20 overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-outline-variant/10 dark:border-outline-variant/20 flex items-center gap-2">
                  ...
                  <h3 className="font-black text-on-surface text-sm">الأسعار</h3>
                </div>
                <div className="p-6">
              ...
              <div className="bg-surface-container-lowest dark:bg-surface-container border border-outline-variant/10 dark:border-outline-variant/20 overflow-hidden shadow-sm">
                <div className="px-6 py-4 border-b border-outline-variant/10 dark:border-outline-variant/20 flex items-center gap-2">
                  ...
                  <h3 className="font-black text-on-surface text-sm">التفاصيل</h3>
                </div>
                <div className="p-6 space-y-2.5 text-sm">
                  <div className="flex justify-between items-center py-2 border-b border-outline-variant/10 dark:border-outline-variant/20"><span className="text-on-surface-variant">المزود</span><span className="font-black text-on-surface">{trip.providerName}</span></div>
                  {trip.vehicleType && <div className="flex justify-between items-center py-2 border-b border-outline-variant/10 dark:border-outline-variant/20"><span className="text-on-surface-variant">نوع المركبة</span><span className="font-black text-on-surface">{trip.vehicleType}</span></div>}
                  {trip.capacity && <div className="flex justify-between items-center py-2 border-b border-outline-variant/10 dark:border-outline-variant/20"><span className="text-on-surface-variant">السعة</span><span className="font-black text-on-surface">{trip.capacity} مقعد</span></div>}
                  {trip.governorate && <div className="flex justify-between items-center py-2"><span className="text-on-surface-variant">الموقع</span><span className="font-black text-on-surface flex items-center gap-1"><span className="material-symbols-outlined text-primary text-sm">location_on</span> {trip.governorate}{trip.city ? ` - ${trip.city}` : ''}</span></div>}
                </div>
              ...
              {trip.features.length > 0 && (
                <div className="bg-surface-container-lowest dark:bg-surface-container border border-outline-variant/10 dark:border-outline-variant/20 overflow-hidden shadow-sm">
                  <div className="px-6 py-4 border-b border-outline-variant/10 dark:border-outline-variant/20 flex items-center gap-2">
                    ...
                    <h3 className="font-black text-on-surface text-sm">المميزات</h3>
                  </div>
                  <div className="p-6">
                    <div className="flex flex-wrap gap-2">
