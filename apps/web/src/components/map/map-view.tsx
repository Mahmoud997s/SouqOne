...
export default function MapView({
  ...
  const shareText = `📍 ${title || 'موقع الإعلان'}\n${googleMapsUrl}`;
  const whatsappShareUrl = `https://wa.me/?text=${encodeURIComponent(shareText)}`;
  const telegramShareUrl = `https://t.me/share/url?url=${encodeURIComponent(googleMapsUrl)}&text=${encodeURIComponent(title || 'موقع الإعلان')}`;
          ...
          <div className="relative">
            ...
            {shareOpen && (
              <div className="absolute top-full mt-2 right-0 bg-white rounded-xl shadow-xl border border-outline-variant/20 py-2 min-w-[180px] z-50">
                ...
                <button onClick={copyLink} className="flex items-center gap-3 px-4 py-2.5 hover:bg-surface-container-low transition-colors text-sm w-full text-right">
